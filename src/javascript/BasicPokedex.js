function logic() {
    ...
    let main = document.querySelector(".side");
    if (name == "" || type == "" || region == "") {
        alert("Please select or enter an option");
    }
    else {
        const pokemon = { name: name, type: type, region: region };
        let pokemonText = document.createTextNode(`name: ${pokemon.name},type: ${pokemon.type},region: ${pokemon.region}`);
        let pokemonData = document.createElement("span");
        pokemonData.appendChild(pokemonText);
        main.appendChild(pokemonData);
